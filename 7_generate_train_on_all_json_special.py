...
#   * ICI: toutes les séquences TRAIN font exactement 16 s
#   * ICI: chaque séquence TRAIN a entre 1 et 5 s d’ictal
# - TEST (toutes les seizures): exactement 1 séquence par seizure
#   * aussi longue que possible (ou contrôlée)
...
# On veut des séquences TRAIN de longueur 16 (supposé 1 epoch = 1 s)
L_TARGET = 16      # longueur visée par séquence (16 s)
S_TARGET = 1       # stride

# Garder les séquences plus courtes que L_TARGET si crise courte (TRAIN)
# Si tu veux VRAIMENT toujours 16 s, on met KEEP_SHORT = False
KEEP_SHORT = False
MIN_SEQ_EPOCHS = 16   # peu importe si KEEP_SHORT=False, mais gardé pour compatibilité

# Séquence centrée onset (TRAIN)
ADD_ONSET_SEQ = True
# On essaie aussi de construire une séquence centrée onset de longueur 16
ONSET_PRE_TARGET = 8   # 8 avant l’onset
ONSET_POST_TARGET = 8  # 8 après l’onset (onset inclus dans la zone "droite")
ONSET_MIN_TOTAL = 16   # on veut 16 sinon on jette

# Séquence centrée onset (TEST)
# CONTROLLED = None  # mode "max len"
CONTROLLED = (10, 5)    # exemple: 5 pré + 5 ictal (onset inclus) => longueur 10
...
def _count_ictal_in_seq(seq_paths: List[str]) -> int:
    """Compte le nombre de chemins 'ictal' dans la séquence."""
    return sum(_path_has_phase(p, "ictal") for p in seq_paths)
...
def make_sliding_sequences(paths_sorted: List[str],
    ...
    """
    Fenêtres glissantes ; on garde uniquement celles qui:
      - contiennent préictal & ictal
      - ont longueur EXACTE L_target (sauf cas KEEP_SHORT=True)
      - ont entre 1 et 5 epochs ictal
    """
    out = []
    n = len(paths_sorted)
    if n == 0:
    ...
    # Si la crise est plus courte que L_target
    if n < L_target:
        if keep_short and n >= min_short:
            cand = paths_sorted
            if _seq_has_both_phases(cand):
                ictal_count = _count_ictal_in_seq(cand)
                if 1 <= ictal_count <= 5:
                    out.append(cand)
        # Sinon, on ne génère rien (impossible d'avoir 16 s)
        return out

    # On force des fenêtres de longueur EXACTE L_target
    L_eff = L_target
    ...
    while i + L_eff <= n:
        ...
        if not _seq_has_both_phases(cand):
            i += S_eff
            continue
...
def onset_centered_sequence_adaptive(paths_sorted: List[str],
    ...
    """
    TRAIN: UNE séquence centrée t0.
    Ici, on impose en plus (pour respecter la contrainte TRAIN) que:
      - la longueur finale soit exactement L_TARGET
      - la séquence contienne préictal & ictal
      - le nombre d'ictal soit entre 1 et 5

    Si ce n'est pas possible, on renvoie [].
    """
    ...
    # On impose les mêmes contraintes que pour le TRAIN glissant
    if len(seq) != L_TARGET:  # 16
        return []
    if not _seq_has_both_phases(seq):
        return []
    ictal_count = _count_ictal_in_seq(seq)
    if not (1 <= ictal_count <= 5):
        return []
...
def best_centered_onset_sequence_maxlen(
    ...
    if controlled is not None:
        ...
        return seq if _seq_has_both_phases(seq) else []  # pour TEST on ne bride pas le nb d'ictal
...
def three_offset_test_sequences_same_length(
    ...
    def ok_length(L: int, p50_override: Optional[int] = None) -> bool:
        ...
        if p25 > pre_available or (L - p25) > ictal_count:
            return False
        if p50 > pre_available or (L - p50) > ictal_count:
            return False
        if p75 > pre_available or (L - p75) > ictal_count:
            return False
        return True
...
def main():
    ...
    for (dataset, patient), dfP in by_pat:
        ...
        for sid in crises:
            ...
            # TRAIN (séquence centrée onset, optionnelle)
            if ADD_ONSET_SEQ:
        ...
        if PRINT_PER_PATIENT_STATS:
            print(f"[OK] {pat_key}: crises={len(crises)} | "
                  f"train_seqs={len(bucket_train)} | test_seqs={len(bucket_test)}")

...
uv run z_visualise_final_performances_F1.py \
  --root /home/julieletallec/test/results_grid_search_kwta_20_10_burst \
  --out figures_grid_search_kwta_20_10_burst_nozerovalF1_specialFP_new0.1_BCE_top10 \
  --select_f1_pct 10
"""
...
def make_patientline_jitterplot_triplet_level(
    ...
    jitter: float = 0.18,   # jitter horizontal en "unités patient" (0.15-0.25 bien)
    alpha: float = 0.25,
    s: float = 8,
):
    """
    ...
    for _, row in df_best.iterrows():
        ...
        # identifiant électrode (juste pour rester cohérent avec ton wording)
    ...
    for ax, (col, ax_title) in zip(axes, metrics):
        ...
        # labels en 2 lignes comme tes autres plots
        xtick_labels = []
        for p in patients:
            if "__" in p:
...
def _collapse_group_by_node(df_group: pd.DataFrame) -> pd.DataFrame:
    ...
    # si rank absent, on peut le recalculer au besoin (mais ici pas obligatoire)
    return df_c
...
def precision_recall_f1_topk(
    ...
    if fp_mode == "classic":
        ...
        if tp == 0:
            fp = k  # aucun SOZ trouvé -> tout le top-k est FP
        else:
            # position (0-based) du dernier SOZ dans top-k
            last_soz_pos = int(np.where(top_labels == 1)[0].max())
            # FP = nonSOZ strictement avant cette position
...
def compute_patient_metrics_from_predictions_df(df_pat: pd.DataFrame) -> Dict[str, float]:
    ...
    for _, g in grp:
        ...
        # drop rows with NaN score (can't rank)
        ...
        # group-level means
        if (labels == 1).any():
            pos_mean = float(np.mean(scores[labels == 1]))
        else:
            pos_mean = np.nan
        if (labels == 0).any():
            neg_mean = float(np.mean(scores[labels == 0]))
        else:
            neg_mean = np.nan
        ...
        # AUC per-group if both classes present
    ...
    for results_dir in find_results_dirs(root):
        ...
        for exp_dir in sorted(clf_root.iterdir()):
            ...
            for pat in sorted(df_pred["patient"].unique()):
                ...
                # contrainte min SOZ par seizure
...
def _infer_electrode_label_col(df: pd.DataFrame) -> str:
    ...
    if "node_index" in df.columns:
        return "node_index"
    return ""  # fallback: on utilisera l'index
...
def plot_scores_per_seizure_per_patient_chum(
    ...
    for _, row in dfb.iterrows():
        ...
        # électrode id (node_index sinon fallback)
        if "node_index" not in df_pat.columns:
            df_pat = df_pat.reset_index(drop=False).rename(columns={"index": "node_index"})

        # colonne label électrode à afficher
        label_col = _infer_electrode_label_col(df_pat)
        if label_col == "":
            # fallback: string de node_index
        ...
        # --- agrégation "1 point par électrode et par seizure"
        # si plusieurs seq_idx / duplications: max y_score, is_SOZ OR
        agg = (
        ...
        # order seizures
        # on trie "numériquement" si possible, sinon lexicographiquement
        ...
        seizure_to_x = {sid: i + 1 for i, sid in enumerate(seizure_ids_sorted)}  # 1..N
        ...
        if annotate != "none":
            # on annote par seizure (utile pour limiter le nombre de labels)
            for sid in seizure_ids_sorted:
                ...
                # option: limiter nb labels / seizure (pour ne pas surcharger)
                if max_labels_per_seizure is not None and len(sub) > int(max_labels_per_seizure):
                    # garde les plus gros scores
        ...
        ax.set_ylim(0, 1)  # adapte si tes scores sortent de [0,1]
        ax.set_xticks(range(1, len(seizure_ids_sorted) + 1))
        ax.grid(axis="y", alpha=0.3)

        # petit rappel x->seizure_id si besoin
        # (optionnel: ça peut être long)
        # ax.set_xticklabels([str(s) for s in seizure_ids_sorted], rotation=90, fontsize=8)
...
def make_boxplot(
    ...
    """
    Version make_boxplot alignée sur plot_confidence_gap_boxplot (ta référence):
      - ax.boxplot(... tick_labels=..., showfliers=False, widths=0.30, patch_artist=True, styles ...)
      - points jitterés (même couleur sur les 3 colonnes)
      - légende à droite
      - tight_layout()
      - savefig(... bbox_inches="tight")

    IMPORTANT:
      - Ici on n'utilise PAS canon_patient, car ton color_map est basé sur df_best["patient"].
    """
    ...
    # -> on garde 1 entrée par patient, triés (comme ta référence)
    pats = sorted(dfp["patient"].drop_duplicates().tolist())

    handles = []
    for pat in pats:
        label = patient_labels_2lines(pat)
        handles.append(
            Line2D(
                [0], [0],
                marker="o",
                color="none",
                markerfacecolor=color_map.get(pat, "grey"),
    ...
    #fig.subplots_adjust(right=0.85)   # réserve la zone de la légende
    plt.savefig(fig_path, dpi=200)   # <-- PAS de bbox_inches="tight"
    plt.close()
...
# ----------------------------------------------------------
# 4) Figure top-k metrics (MODIF: ajoute F1 top10 & top20)
# ----------------------------------------------------------
...
def make_boxplot_topk_metrics(df: pd.DataFrame, out_dir: Path, color_map, title: str):
    ...
    #plt.figure(figsize=(max(10, len(present) * 1.25), 6))
...
# ----------------------------------------------------------
# 5) Rang du PREMIER SOZ + counts (inchangé, basé sur best combo)
# ----------------------------------------------------------
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
def plot_soz_rank_mix_first_and_all(
    ...
    point_size: float = 70,
    capsize: float = 4,
    x_offset: float = 0.14,      # décalage gauche/droite des 2 points
    bg_width: float = 0.82,      # largeur des barres de fond
):
    """
    Mix des 2 plots:
    ...
    Entrées attendues :
      df_first: output de compute_first_soz_rank_and_counts
      df_all  : output de compute_all_soz_rank_and_counts
    """

    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    # --- merge (safe) sur patient
    df = pd.merge(
        df_first,
        df_all,
        on="patient",
    ...
    # --- fond: total electrodes + SOZ electrodes
    ...
    # nb seizures (on prend celui de df_all par défaut)
    ...
    # couleurs des deux séries (simples, lisibles)
    # -> first: noir ; all: blanc (ou inverse). Ici: first = noir, all = blanc.
    ...
    if show_seizure_count:
        # place tout en haut, basé sur hauteur max du fond
    ...
    # xticks 2 lignes comme chez toi
    xtick_labels = []
    for p in patients:
        if "__" in p:
            part1, part2 = p.split("__", 1)
    ...
    # limites y: doit couvrir ranks ET fonds
    y_max_candidates = [
...
def compute_first_soz_rank_and_counts(df_best: pd.DataFrame, root: Path) -> pd.DataFrame:
    ...
    for _, row in df_best.iterrows():
        ...
        csv_path = (
            root / res_dir / big_exp_name / exp_dir / "cv_val_predictions_ranked.csv"
        )
...
# ----------------------------------------------------------
# 6) Rang de TOUTES les électrodes SOZ (inchangé, basé sur best combo)
# ----------------------------------------------------------
...
def compute_all_soz_rank_and_counts(df_best: pd.DataFrame, root: Path) -> pd.DataFrame:
    ...
    for _, row in df_best.iterrows():
        ...
        for _, g in grp:
            ...
            # max SOZ electrodes distinct in this group
...
def onset_curve_bce(p_smooth, t_true, eps=1e-7):
    ...
    p_smooth : array-like (T,)
    t_true   : index entier (None/<0 => NaN)
    """
...
def collect_bce_from_npz_for_best_models(root: Path, df_best: pd.DataFrame) -> pd.DataFrame:
    """
    Pour chaque patient (best results_dir/experiment dans df_best),
    parcourt results/<patient>/series/*.npz et calcule BCE vs step(onset).
    """
    rows = []
    for _, row in df_best.iterrows():
        pat = str(row["patient"])
        ...
        for npz_path in sorted(series_dir.glob("*.npz")):
            ...
            # selon tes fichiers, ça peut être p_graph_smooth ou p_graph
            if "p_graph_smooth" in data:
                p_smooth = np.asarray(data["p_graph_smooth"], dtype=float)
            elif "p_graph" in data:
                # fallback si tu n’as pas p_graph_smooth dans ces npz
...
def keep_middle_seq_per_seizure(df_bce: pd.DataFrame) -> pd.DataFrame:
    """
    Pour chaque (patient, seizure_id), garde la séquence dont seq_index est au milieu.
    (fonctionne même si != 3 séquences)
    """
    def _pick_middle(g):
...
def plot_bce_boxplot_per_patient(df_bce: pd.DataFrame, out_dir: Path, color_map: dict):
    ...
    Plus la BCE est faible, meilleure est la correspondance onset.
    """
...
def compute_confidence_metrics_topk_for_best_models(
    ...
    root: Path,
    frac: float = 0.10,
    fp_mode: str = "before_last_soz",  # pas utilisé ici, mais je laisse si tu veux homogénéiser plus tard
) -> pd.DataFrame:
    """
    Pour chaque patient (best model dans df_best):
      - charge cv_val_predictions_ranked.csv
      - filtre le patient
      - groupe par (patient,seizure_id,seq_idx)
      - collapse par node_index (via _collapse_group_by_node)
      - garde uniquement les électrodes top-k (frac) selon y_score dans chaque groupe
      - calcule pour chaque groupe:
          pos_mean = mean(y_score des SOZ dans top-k)
          neg_mean = mean(y_score des non-SOZ dans top-k)
          gap      = pos_mean - neg_mean
      - agrège patient-level = nanmean sur les groupes

    Retourne un DF: patient, mean_score_pos, mean_score_neg, confidence_gap
    """
    rows = []
...
def plot_top10_scores_distribution_per_patient(
    ...
    """
    Pour chaque patient (best model dans df_best), collecte tous les y_score des électrodes
    appartenant au top-k (frac) AU SEIN de chaque groupe (patient,seizure_id,seq_idx),
    après collapse par node_index.

    Plot: boxplot par patient + points jitter.
    """
    ...
    for _, row in df_best.iterrows():
        ...
        if not all(k in df_pat.columns for k in group_keys):
            print(f"[WARN] Missing group_keys in {pred_path} for {pat}")
            continue
    ...
    # Boxplot (matplotlib) for consistency with your style
    data = []
    for p in patients:
    ...
    # Points jitter (option: shape differs for SOZ vs non-SOZ)
    # Here: cercle = non-SOZ, triangle = SOZ (facultatif, mais pratique)
    for i, p in enumerate(patients, start=1):
        ...
        # SOZ electrodes → rouge et un peu plus gros
        m1 = (labs == 1) & np.isfinite(vals)
        ax.scatter(
            xs[m1],
            vals[m1],
            s=point_size * 1.6,      # un poil plus gros
            alpha=0.9,
            color="red",             # rouge pour SOZ
            marker="o",              # cercle (ou "^" si tu préfères)
            linewidth=0,
            zorder=3,                # au-dessus des autres
        )
...
def main():
    ...
    # Figure top-k metrics (MODIF: inclut F1 top10 & top20)
    ...
    # Figure boxplot SOZ ranks points colored by seizure + background
    ...
    # si tu veux exactement la même convention que ton autre script :
